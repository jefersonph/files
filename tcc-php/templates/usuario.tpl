							...
							<table width="100%" border="0" cellspacing="0" cellpadding="5" bordercolor="#FFFFFF" id="ApplicationAdministratorTable">
								...
								<tr>
									<td class="actionbkgrd">
										<a class="roleaction" href="index.php?op=c_usuario"><img width="16" height="13" hspace="8" border="0" src="img/action.gif">Criar</a>
									</td>
								</tr>
								{/if}
								<tr>
...
<script language="javascript">
	...
	$('#empresas option').live('click', function(){
		$.ajax({
                type: "POST",
				url: "receber.php",
				data: {operacao:"listar_usuarios", empresa:$(this).val()},
				success: function(msg){
				$("#results2").html(msg);
			}
        });
...
<script language="javascript">
		$.ajax({
                type: "POST",
				url: "receber.php",
				data: {operacao:"listar_usuarios", empresa:"false"},
				success: function(msg){
				$("#results").html(msg);
			}
        });
