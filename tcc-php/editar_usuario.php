...
<script language="javascript">
	var idusuario = <?php echo $_GET['id']; ?>;
	$.ajax({
			type: "POST",
			url: "receber.php",
			data: {operacao:"editar_usuario", idusuario: idusuario},
	...
	$('#salvar').live('click', function() {	
		$.ajax({
                type: "POST",
				url: "receber.php",
				data: {operacao:"atualizar_usuario", id: idusuario, login:$('#login').val() ,nome_user:$('#nome_user').val(), senha:$('#senha').val(),email:$('#email').val(), cpf:$('#cpf').val(), permissao:$('#permissao').find('option').filter(':selected').text() },
				success: function(msg){
				$("#results3").html(msg);
			}
        });
