...
<script language="javascript">
	$(function() {
	var idservidor = <?php echo $_GET['id']; ?>	
	$.ajax({
			type: "POST",
			url: "receber.php",
			data: {operacao:"editar_servidor", idservidor: idservidor},
	...
	$('#salvar').live('click', function() {
		$.ajax({
                type: "POST",
				url: "receber.php",
				data: {operacao:"atualizar_servidor", id: idservidor ,ip:$('#ip').val(), mascara:$('#mascara').val(), arquitetura:$('#arquitetura').val(), so:$('#so').val(), descricao:$('#descricao').val()},
				success: function(msg){
				$("#results3").html(msg);
			}
        });
