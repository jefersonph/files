...
<tr>
        ...
        <td width="95%" valign="top">
				...
				<div class="edit">
					<div  id="verificar" class="button">Verificar Arquivo</div>
					<select>
...
$('.edit').find('option').filter(':selected').text()
var arquivo = ("<?php echo $file; ?>");
var arquivo2 = ("<?php echo $file2; ?>");
$('#salvar').live('click', function() {
	$.ajax({
		type: "POST",
		url: "receber.php",
		data: {operacao:"salvar_arquivo", ckeditor: CKEDITOR.instances.editor1.getData(), file:arquivo, file2:arquivo2},
...
$('#verificar').live('click', function() {
	$.ajax({
		type: "POST",
		url: "receber.php",
		data: {operacao:"verificar_sintaxe", service: $('.edit').find('option').filter(':selected').text(), file: CKEDITOR.instances.editor1.getData() },
		success: function(msg){
			$("#results").html(msg);
		}		
	});
